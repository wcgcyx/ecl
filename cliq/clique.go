...
import (
	...
	"github.com/wcgcyx/ecl/constant"
...
func encodeSigHeader(w io.Writer, header *types.Header) {
	...
	if header.ParentBeaconRoot != nil {
		// CHANGE(immutable): Allow Cancun
		if *header.ParentBeaconRoot != constant.MinHash {
...
func enforceCancunHeaderInvariants(header *types.Header) error {
	...
	if header.ParentBeaconRoot.Cmp(constant.MinHash) != 0 {
		return fmt.Errorf("invalid parentBeaconRoot, have %#x, expected zero hash", header.ParentBeaconRoot)
	}
	return nil
}

// CHANGE(immutable): AuthorizeWithPrivateKey will authorize the signing key.
func (c *Clique) AuthorizeWithPrivateKey(signer common.Address, signFn SignerFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
// CHANGE(immutable): SealECL seals block, should only be called by ECL.
func (c *Clique) SealECL(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	isCancun := true // chain.Config().IsCancun(header.Number, header.Time)
	if isCancun {
		if err := enforceCancunHeaderInvariants(header); err != nil {
